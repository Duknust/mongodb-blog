...
public class UserDAO {
    ...
    public boolean addUser(String username, String password, String email) {
        ...
        if (email != null && !email.equals("")) {
            insert = new BasicDBObject("_id",username).append("username", username).append("password", passwordHash).append("email", email);
            // XXX WORK HERE
            // if there is an email address specified, add it to the document too.
        } else {
            insert = new BasicDBObject("_id",username).append("username",username).append("password", passwordHash);
        }

        try {
            System.out.println("\n\n"+insert+"\n\n");
            usersCollection.insert(insert);
            // XXX WORK HERE
